...
module.exports = {
   ...
   * @example
   *
   * const command = require('pkg-update');
  ...
  async askForWhatUserWants() {
    ...
    let [res, err] = await cp.exec(`cd ${projectDir} && npm ls --parseable${isDev ? '' : ' --prod'}`);
    spinner.stop();
    if (err) {
      throw new Error(err);
    }
    ...
    spinner.start();
    [res, err] = await cp.exec(`npm view ${pkgName} versions --json`);
    spinner.stop();
    if (err) {
      throw new Error(err);
    }
  ...
  async runUpdateCheck(conf) {
    ...
    const [res, err] = await cp.exec(`cd ${projectDir} && npm ls ${pkgName} --json${isDev ? '' : ' --prod'}`);
    if (err) {
      spinner.stop();
      throw new Error(err);
    }
